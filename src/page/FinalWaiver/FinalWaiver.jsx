...
const FinalWaiver = () => {
    ...
    useEffect(() => {
        ...
        // Extract all programs from the JSON data
        const allPrograms = [];
        const allFaculties = [];
    ...
    const isFormValid = () => {
        ...
        if (!formData.program || !formData.gender || !formData.educationBoard) {
            return false;
        }
    ...
    const calculateWaiver = async () => {
        ...
        // Calculate Female Quota (10% for female students)
        ...
        if (formData.eduStatus === 'general') {
            ...
            // Get faculty from selected program
            const facultyId = selectedProgram?.facultyId;

            // Faculty of Humanities & Social Sciences (ID: 105)
            if (facultyId === 105) {
                if (hsc >= 4.75 && hsc <= 4.89) sf_waiver = 10;
                else if (hsc >= 4.9 && hsc <= 4.99) sf_waiver = 15;
                else if (hsc === 5.0) sf_waiver = 20;

                if (ssc === 5.0 && hsc === 5.0) sf_waiver = 25;
                ...
                if (hsc_g === 1 && ssc_g === 1) {
                    ...
                    sf_waiver = 30;
                } else if (hsc === 5.0 && ssc === 5.0) {
                    sf_waiver = 25;
                } else if (hsc === 5.0) {
                    sf_waiver = 20;
                } else if (hsc >= 4.9 && hsc <= 4.99) {
                    sf_waiver = 15;
                } else if (hsc >= 4.5 && hsc <= 4.89) {
                    sf_waiver = 10;
                }
            }
        }
        ...
        if (formData.eduStatus === 'diploma' && formData.dipGpa) {
        ...
        // SUM all applicable waivers (not taking max)
        // Start with result-based waiver
        let finalWaiver = sf_waiver + d_waiver;

        // Add female quota if applicable (always added)
        finalWaiver += f_waiver;

        // Add player waiver if applicable
        finalWaiver += p_waiver;

        // Cap at 100% maximum
        finalWaiver = Math.min(finalWaiver, 100);
                        ...
                        <div className="space-y-6">
                            ...
                            <div className="space-y-4">
                                ...
                                {formData.eduStatus === 'general' ? (
                                    <>
                                        <div className="grid grid-cols-1 md:grid-cols-2 gap-3 no-animation-grid">
                                            {/* SSC Result */}
                    ...
                    {isCalculating ? (
                        <div className="bg-linear-to-tr from-slate-50 via-white to-indigo-50 rounded-2xl shadow-lg p-6 border border-gray-200 h-full flex items-center justify-center lg:mt-12">
                            ...
                            <div className="space-y-3">
                                ...
                                {result.breakdown.current > 0 && (
                                    <div className="flex items-center justify-between p-3 bg-teal-50 rounded-xl border border-teal-100">
                                        <div className="flex items-center gap-3">
                                            <Users className="w-5 h-5 text-teal-600" />
                                            <span>Current Student Waiver</span>
                                        </div>
                                        <span className="font-bold text-teal-600">{result.breakdown.current}%</span>
                                    </div>
                                )}
                            </div>
